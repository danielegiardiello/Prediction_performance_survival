...
brier_score<-function(tfup,status,thorizon,survival) {
  ...
  db<-data.frame(time=tfup,
                 status=status)
  
  db$predsurv<-survival
  db$predmort<-1-db$predsurv
  
  db<-db %>% mutate(cat=case_when(time<=thorizon & status==1~1,
                                  time>thorizon ~2,
  ...
  db2 <- db %>% left_join(sfit_df,by="time") %>% 
  ...
  sfit_null_df<-data.frame(time=c(0,sfit_null$time),
                           surv=c(1,sfit_null$surv))

  db_null2 <- db_null %>% left_join(sfit_null_df,by="time") %>% 
