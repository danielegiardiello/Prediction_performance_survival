...
# NOTE: this can be removed, probably Table 1 is unneccesary in the very simplified code
# Summary guidelines of Terry
# 1. Nice output may be as a matrix with rownames and colnames
# 2. Provide sections mostly stand alone with dataset setup and libraries needed for
# every section
# 3. Use survival package and dnypred (e.g. pec)


## Additional NOTE:
# Hmisc:rcs() is used in the survival::coxph() to allow rcs with 3 knots
# Bootstrap: can we use rsample::bootstraps() or a classical way avoiding tidyverse?
# Brier: dynpred:pecox() does not allow to estimate Brier score for external validation 
# can we use riskRegression::Score() ?
# Concordance: do you agree with timeROC::timeROC()?
# Calibration: Austin is OK using survival::coxph() and OE ratio is calculated using an ad-hoc function using survival and pec:predictSurvProb() package
...
# NOTE: dynpred::pecox() does not allow for external validation?
...
# res_ov_lab <- c("", "Apparent", "", "", "Apparent + PGR", "",
#                 "", "External", "", "", "External + PGR", "")
# res_ov <- rbind(res_ov_lab,res_ov)
res_ov
```

```{r, overall_table, warning=FALSE, echo=FALSE}
# Table overall measures
alpha <- .05
k <- 2 # number of digits
res_ov <- matrix(unlist(c(
