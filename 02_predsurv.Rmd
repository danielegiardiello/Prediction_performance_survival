...
OE_ratio5 <- OE.ratio.t(
  tfup = gbsg5$ryear, event = gbsg5$rfs,
...
# Calibration slope
calslope <- coxph(Surv(ryear, rfs) ~ lp, data = gbsg5)
calslope_1b <- coxph(Surv(ryear, rfs) ~ lp_1b, data = gbsg5)
```
...
gbsg5$predmort5 <- 1 - gbsg5$predsurv5
gbsg5$predmort5.cll <- log(-log(1 - gbsg5$predmort5))

# Estimate
vcalibrate.efit1 <- cph(Surv(ryear, rfs) ~ rcs(predmort5.cll, 3), x = T, y = T, data = gbsg5, surv = T) # better rms::cph?
predict.grid <- seq(quantile(gbsg5$predmort5, prob = 0.01), quantile(gbsg5$predmort5, prob = 0.99), length = 100)
...
# par(new=T)
# plot(density(gbsg5$predmort5),
#     axes=F,xlab=NA,ylab=NA,
#     main="")


## External data with PGR
gbsg5$predmort5 <- 1 - gbsg5$predsurv5_1b
gbsg5$predmort5.cll <- log(-log(1 - gbsg5$predmort5))

# Estimate
vcalibrate.efit1b <- cph(Surv(ryear, rfs) ~ rcs(predmort5.cll, 3), x = T, y = T, data = gbsg5, surv = T) # better rms::cph?
predict.grid <- seq(quantile(gbsg5$predmort5, prob = 0.01), quantile(gbsg5$predmort5, prob = 0.99), length = 100)
predict.grid.cll <- log(-log(1 - predict.grid))
predict.grid.df <- data.frame(predict.grid)
predict.grid.cll.df <- data.frame(predict.grid.cll)
...
gbsg5$group1 <- cut(gbsg5$lp, breaks = quantile(gbsg5$lp, probs = seq(0, 1, 0.25)), include.lowest = TRUE)
...
gbsg5$group1b <- cut(
  gbsg5$lp_1b,
  breaks = quantile(gbsg5$lp_1b, probs = seq(0, 1, 0.25)),
  include.lowest = TRUE
)
# gbsg5$group1b<-cut(gbsg5$lp1b,breaks=quantile(gbsg5$# lp1b,probs=seq(0,1,0.25)),include.lowest = TRUE)

par(las = 1, xaxs = "i", yaxs = "i")
plot(survfit(Surv(ryear, rfs) ~ group1b, data = gbsg5),
  bty = "n", xlim = c(0, 5),
