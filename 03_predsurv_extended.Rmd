...
```{r, discr_time, fig.align='center', warning=FALSE, eval=TRUE}
```
...
+ Mean calibration can be estimated:  
  ...
  + at a fixed time point:
    + using the Observed and Expected ratio at time t. The estimated Observed number of events is calculated by the Kaplan-Meier curve, and the Expected number of events iscalculated as the average of the estimated predicted risk at time _t_ (e.g. 5 years);  
...
+ Moderate calibration can estimated:
  ...
  + time range: 
    + Plot the observed / expected number of events over time  using Poisson model intercept, exp(a) with log of cumulative hazard as offset across all time points to _t_ (i.e. 5 years);  
...
res_OE <- matrix(c(
  ...
  int_val["Mean calibration fixed time point - OE", ],
  NA,
  NA,
  ...
  int_val_pgr["Mean calibration fixed time point - OE", ],
  NA,
  NA,
  OE_pgr,
  OE_pgr * exp(-qnorm(1 - alpha / 2) * sqrt(1 / obj$n.event)),
  OE_pgr * exp(qnorm(1 - alpha / 2) * sqrt(1 / obj$n.event))
  ),
...
res_cal <- matrix(c(
  ...
  int_val_pgr["Mean calibration corrected - time range", ],
  NA,
  NA,
  
  exp(int_summary_pgr[1]),
  exp(int_summary_pgr[1] - qnorm(1 - alpha / 2) * int_summary_pgr[2]),
    exp(int_summary_pgr[1] + qnorm(1 - alpha / 2) * int_summary_pgr[2])
),
...
res_cal <- matrix(c(
  int_val["Weak calibration corrected (slope) - fixed time point", ],
  NA,
  NA,
  ...
  int_val_pgr["Weak calibration corrected (slope) - fixed time point", ],
  NA,
  NA,
                    
  gval_prs$coef,
  gval_prs$coef - qnorm(1 - alpha / 2) * sqrt(gval$var),
  gval_prs$coef + qnorm(1 - alpha / 2) * sqrt(gval$var) 
  ),
  nrow = 1,
  ncol = 12,
...
res_cal <- matrix(c(
  ...
  int_val_pgr["Weak calibration corrected (slope) - time range", ],
  NA,
  NA,
...
```{r, cumhaz_plot, message=FALSE,warning=FALSE, fig.align='center', fig.width = 6, fig.height = 6, echo=FALSE, eval=FALSE, include=FALSE}
...
```{r, cumhaz_plot, fig.align='center', fig.width = 6, fig.height = 6, warning=FALSE, eval=FALSE, include=FALSE}
```
