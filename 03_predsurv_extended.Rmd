...
```
Calibration intercept was 0.15 and 0.14 for the basic and extended model, respectively.  
...
fit1 <- glm(rfs ~ offset(p), family=  poisson, data = gbsg5,
            subset = (p > 0))
fit2 <- glm(rfs ~ lp + offset(logbase), family = poisson, 
            data = gbsg5,
            subset = (p > 0))


group <- cut(lp, c(quantile(lp, probs = seq(0, 1, 0.25))))
