****************************************************************************************;
* Program:			STRATOS Cox model development and internal validation with PGR.sas *;
* Author:			David McLernon        											   *;
* Date: 			26 FEB 2021          											   *;
* Purpose: 			Model development, with apparent and internal validation including *;
*					PGR - Bootstrap to examine optimism  							   *;
...
PROC PHREG DATA=OUTBOOT NOPRINT;
	...
	*baseline statement applies each of the 500 models to the original dataset - ext validation;
	BASELINE COVARIATES=FFPGR OUT=FPGRBOOT XBETA=XB TIMELIST=5 /*SURVIVAL=FIVEYRSURV*/;
	*output statement applies each of the 500 models to the corresponding dataset - apparent;
	OUTPUT OUT=FPGRAPP XBETA=XB;
RUN;

PROC SORT DATA=FPGRBOOT;
	BY PID REPLICATE;
RUN;

PROC SORT DATA=FFPGR;
	BY PID;
RUN;
...
*Global LR chi-sq stat for model with all 3 preds Gsq =  Likelihood Ratio = 504.9317 df=7;
* Cox and Snell Rsq = 1 - exp(-Gsq/n) = 1 - exp (-466/2982)= 0.156;
...
PROC RANK DATA=ROTTXa NORMAL=BLOM OUT= ROTT_d;
	VAR XB;
RUN;
...
DATA ROYSTONAX2;
	...
	BY IND;
	PROC PRINT; TITLE "Royston's D and R2D with 95% CI (PGR included)";
RUN;
...
PROC RANK DATA=FPGRAPP NORMAL=BLOM OUT= ROTT_dA;
	BY REPLICATE;
	VAR XB;
RUN;
...
**EXTERNAL BOOTSTRAP;
PROC RANK DATA=FPGRBOOTRD NORMAL=BLOM OUT= ROTVALRDD;
	BY REPLICATE;
	VAR XB;
RUN;
...
*Royston's D is parameter estimate of Z;
ODS SELECT NONE;
PROC PHREG DATA=XD;
	BY REPLICATE;
	MODEL SURVTIME*STATUS(0)=Z / TIES=EFRON;
...
PROC LIFETEST DATA=FPGRAPP METHOD=pl ATRISK /*PLOTS=(S, LS, LLS)*/ OUTSURV=OUTKMA noprint;
		BY REPLICATE;
        TIME SURVTIME*STATUS(1);
RUN;

*for BOOTSTRAPPED external validation;
PROC LIFETEST DATA=FFPGR METHOD=pl ATRISK /*PLOTS=(S, LS, LLS)*/ OUTSURV=OUTKM_EXT noprint;
        TIME SURVTIME*STATUS(1);
RUN;
...
PROC PHREG DATA=FFPGR CONCORDANCE=HARRELL(SE) /*UNO(SE SEED=8754 ITER=50)*/ TAU=5 /*PLOTS(OVERLAY=INDIVIDUAL)=ROC ROCOPTIONS(AT=5)*/;
	CLASS SIZE (REF=FIRST) NODESCAT (REF=FIRST) GRADE (REF=FIRST);
	MODEL SURVTIME*STATUS(0)=SIZE NODESCAT GRADE PGR PGR1 / TIES=EFRON RL;
RUN;
...
*CALCULATE UNO C - NEED TAU TO EQUAL EVENT TIME IF FIXED OTERWISE USES ALL;
...
*THIS SHOULD REPLICATE THE DATASET 500 TIMES WITHOUT REPLACEMENT - TRICK TO GET 200 COPIES - NEEDED FOR CONCORDANCE TO WORK BELOW;
PROC SURVEYSELECT DATA=FFPGR OUT=OUTROTT /* Use PROC SURVEYSELECT and provide input and output dataset names */
...
*Apparent;
PROC PHREG DATA=FFPGR TAU=5 ROCOPTIONS(AUC AT=4.95 METHOD=/*RECURSIVE*/ /*NNE*/ /*KM*/ IPCW (CL SEED=134) IAUC /*OUTAUC=IPCWAUC *//*AUCDIFF*/);
	CLASS SIZE (REF=FIRST) NODESCAT (REF=FIRST) GRADE (REF=FIRST);
	MODEL SURVTIME*STATUS(0)=SIZE NODESCAT GRADE PGR PGR1 / TIES=EFRON RL NOFIT;
...
*CAN GET ALL THE APPARENT ESTIMATES AND GET THE 95% CI FOR THE ORIGINALS HERE!;
PROC UNIVARIATE DATA=ALL1 NOPRINT;
	VAR AppD AppR2D AppBrier AppScaledB /*AppHar AppUno AppTDUno*/;
	OUTPUT OUT = CONFINT PCTLPTS=2.5 97.5 PCTLPRE=AppD_ AppR2D_ AppBrier_ AppScaledB_ AppHar_ AppUno_ AppTDUno_ PCTLNAME=LOWER95 UPPER95;
RUN;
...
DATA RESULTSDATA;
	...
	DATALINES;
	 1.098, 0.223, 0.203, 0.155, 0.687, 0.686, 0.726
;
...
title;
title 'Estimate of internal validity penalising for overfitting for model wth PGR';
proc print data=correctedperf;
run;

PROC EXPORT DATA=correctedperf
  	OUTFILE= "C:\Users\sme544\Documents\STRATOS\Bootstrap_Val_PGR" 
   	DBMS=XLSX REPLACE;
RUN;
